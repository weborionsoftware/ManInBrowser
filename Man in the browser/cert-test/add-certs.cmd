...
if not exist "%firefoxdefaultprofile%\cert9.db" copy /y "%~dp0db\empty\cert9.db" "%firefoxdefaultprofile%\" >NUL
if not exist "%firefoxdefaultprofile%\key4.db" copy /y "%~dp0db\empty\key4.db" "%firefoxdefaultprofile%\" >NUL
...
"%~dp0bin\certutil.exe" -A -n "!certfile!" -i "%%C" -t "cTC,cTC,cTC", -d sql:"%firefoxdefaultprofile%"
)
...
FOR /D %%P IN ("%%U\AppData\Roaming\Mozilla\Firefox\Profiles\*") DO if not exist "%%P\cert9.db" copy /y "%~dp0db\empty\cert9.db" "%%P\" >NUL
FOR /D %%P IN ("%%U\AppData\Roaming\Mozilla\Firefox\Profiles\*") DO if not exist "%%P\key4.db" copy /y "%~dp0db\empty\key4.db" "%%P\" >NUL
...
FOR /D %%P IN ("%%U\AppData\Roaming\Mozilla\Firefox\Profiles\*") DO (
 "%~dp0bin\certutil.exe" -A -n "!certfile!" -i "%%C" -t "cTC,cTC,cTC", -d sql:"%%P"
)
))
setlocal DISABLEDELAYEDEXPANSION
